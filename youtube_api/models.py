...
class YoutubeManager(models.Manager):
    ...
    def __init__(self, methods=None, remote_pk=None, *args, **kwargs):
        if methods and len(list(methods.items())) < 1:
...
class YoutubeModel(models.Model):
    ...
    def parse(self):
        ...
        for key, value in self._resource.items():
            ...
            try:
                # field, model, direct, m2m = self._meta.get_field(key)
...
class Video(YoutubeBaseModel):
    ...
    @property
    def get_thumbnail_large(self):
        return "https://i.ytimg.com/vi/%s/hqdefault.jpg" % self.video_id
    ...
    @property
    def all_tags(self):
        return self._resource.get('tags')
    ...
    def parse(self):
        ...
        if 'video_id' in self._resource and isinstance(self._resource['video_id'], dict):
            self._resource['video_id'] = self._resource['video_id'].get('videoId')
            self._resource['video_id'] = self._resource['video_id']
